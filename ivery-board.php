...
$board_table_data = $db->query("SELECT * FROM tbl_galaxy_ivery_board ORDER BY board_added_date_time 
	DESC");
...
if (isset($_GET['editid'])) {
	...
	$edit_array = array('id' => $edit_id, 'status' => $status);
	$edit_quary = $db->query("UPDATE tbl_galaxy_ivery_board SET board_status = :status WHERE board_auto_id = :id", $edit_array);

	if ($edit_quary) {
		$successMessage = "Data Updated Successfully.!";
		setcookie("editsuccessMessage", $successMessage, time() + (20 * 1), "/"); // 1 minute
		header("Location:".HTTP_PATH."ivery-board");
	}else {

		$errorMessage = "Error in Updating Data.!";
		setcookie("editerrorMessage", $errorMessage, time() + (20 * 1), "/"); // 1 minute
		header("Location:".HTTP_PATH."ivery-board");
	}
}
